...
$req = $db->prepare("INSERT INTO `verfi_extincteur` (`date_verif`, `type`, `pression_relevee`, `chargeur_ref`, `poids_max`, `poids_min`, `tare`, `poids_gaz`, `a_fixer`, `panneau`, `num_ext_pan`, `ch_a_faire`, `rep_a_faire`, poids_mesurer, `commentaire`, `id_extincteur`, id_verification, `id_user`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
$result = $req->execute(array($date_verif, $type_verif, $pression_releve, $chargeur_ref, $poids_max, $poids_min, $tare, $poids_gaz, $a_fixer, $panneau, $num_ext_pan, $ch_a_faire, $rep_a_faire, $poids_mesurer, $commentaire, $id_ext, $id_verif, $_SESSION['id_vigilus_user']));
$nbr = $req->rowCount();
