...
FROM `ria_pia`
WHERE ria_pia.type='PIA' AND ria_pia.etat=1 AND ria_pia.id_site=? AND ria_pia.id NOT IN (SELECT ria_pia.id FROM `ria_pia` INNER JOIN verfi_ria ON ria_pia.id=verfi_ria.id_ria WHERE ria_pia.etat=1 AND ria_pia.id_site=? AND verfi_ria.id_verification=?)");
$req_ria->execute(array($id_site, $id_site, $id_verif));

$req_ria_attente = $db->prepare("SELECT ria_pia.id, CONCAT(DATE_FORMAT(ria_pia.date_ajout, '%d'), '/', DATE_FORMAT(ria_pia.date_ajout, '%m'),'/', DATE_FORMAT(ria_pia.date_ajout, '%Y')), ria_pia.emplacement, ria_pia.type, ria_pia.marque,  CONCAT(DATE_FORMAT(ria_pia.date_derniere_verif, '%d'), '/', DATE_FORMAT(ria_pia.date_derniere_verif, '%m'),'/', DATE_FORMAT(ria_pia.date_derniere_verif, '%Y')), CONCAT(DATE_FORMAT(verfi_ria.date_verif, '%d'), '/', DATE_FORMAT(verfi_ria.date_verif, '%m'),'/', DATE_FORMAT(verfi_ria.date_verif, '%Y')), verfi_ria.id
FROM `ria_pia`
INNER JOIN verfi_ria ON ria_pia.id=verfi_ria.id_ria
WHERE ria_pia.type='RIA' AND ria_pia.etat=1 AND ria_pia.id_site=? AND verfi_ria.id_verification=?");
$req_ria_attente->execute(array($id_site, $id_verif));
...
<body class="fixed-sn white-skin" style="background-image: url(<?= $image ?>SANGOMAR.jpg);">
    ...
    <div class="    " style="margin-top: -65px;">
        ...
        <section class="section team-section">
            ...
            <div class="row">
                <div class="text-left">
                    <a href="" class="btn btn-primary btn-rounded" data-toggle="modal" data-target="#modalRIA">PIA + </a>
                    <a href="l_site_verif.php?v=<?= $id_verif ?>" class="btn btn-primary btn-rounded">Retour </a>
                </div>
            </div>
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body mb-0">
                                    <form method="POST" action="e_ria_trmnt.php">
                                        ...
                                        <input type="text" name="type_ria" value="RIA" hidden>
                                        <div class="row">
                                            <div class="col-md-4 ">
                                                <div class="md-form">
                                ...
                                <table class="table  table-hover w-auto centered  card-body ml-3">
                                    ...
                                    <tbody>
                                        ...
                                        while ($donnees = $req_ria->fetch()) {
                                            ...
                                            echo "<td class='text-center'><a href='e_ria_va.php?id=" . $id_ria . "&amp;v=" . $id_verif . "' class='btn btn-primary btn-rounded'>VA+</a></td>";
                                            $i++;
                                        ?>
                            ...
                            <div class="row table-responsive ">
                                <table class="table  table-hover w-auto centered  card-body ml-3">
                                    ...
                                    <tbody>
                                        <?php
                                        $i = 1;
                                        while ($donnees = $req_ria_attente->fetch()) {
                                            $id_ria = $donnees['0'];
                                            $date_ajout = $donnees['1'];
                                            $emplacement = $donnees['2'];
                                            $type = $donnees['3'];
                                            $marque = $donnees['4'];
                                            $date_derniere_verif = $donnees['6'];
                                            $id_verif_ria = $donnees['7'];
                                            if ($date_derniere_verif == "") {
                                                $date_derniere_verif = "Pas encore vérifier";
                                            }
                                            //Date dernère vérifiation
                                            $req_ria = $db->prepare("SELECT CONCAT(DATE_FORMAT(verfi_ria.date_verif, '%d'), '/', DATE_FORMAT(verfi_ria.date_verif, '%m'),'/', DATE_FORMAT(verfi_ria.date_verif, '%Y')) FROM verfi_ria WHERE id_ria=? ORDER BY date_verif DESC LIMIT 1");
                                            $req_ria->execute(array($id_ria));
                                            $donnees_ria = $req_ria->fetch();
                                            $nbr_ria = $req_ria->rowCount();
                                            if ($nbr_ria > 0) {
                                                $date_derniere_verif = $donnees_ria['0'];
                                            }
                                            ...
                                            echo "<td class='text-center'>" . $emplacement . "</td>";
                                            echo "<td class='text-center'>" . $date_derniere_verif . "</td>";
                                            echo "<td class='text-center'><a href='m_ria_va.php?id=" . $id_verif_ria . "' class='btn btn-primary btn-rounded'>Détails</a></td>";
                                            $i++;
                                        ?>
