...
$req_va_ria = $db->prepare("INSERT INTO  `verfi_ria`(`date_verif`, `type`, `dn`, `lj`, `vanne_barrage`, `vanne_ria_pia`, `boite_eau`, `devidoir_tambour`, `diffuseur`, `panneau`, `commentaire`, `hs`, `id_verification`, `id_ria`, `id_user`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
$result = $req_va_ria->execute(array($date_verif, $type_verif, $dn, $lj, $vanne_barrage, $vanne_ria_pia, $boite_eau, $devidoir_tambour, $diffuseur, $panneau, $commentaire, $hs, $id_verif, $id_ria, $_SESSION['id_vigilus_user'])) or die(print_r($req_va_ria->errorInfo()));
$nbr = $req_va_ria->rowCount();
